...
export default function Login() {
  ...
  const { authUser, userDoc, loading, signInEmail, signInGoogle } = useAuth();
  ...
  const handleEmailLogin = async () => {
    if (!email || !password) {
      toast({ title: "Missing details", description: "Enter email and password." });
      return;
    }
    ...
    <div className="min-h-screen bg-background flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-muted-foreground mt-2">Sign in to your placement dashboard</p>
        </div>
        ...
        <Card className="card-elevated p-6 space-y-5">
          ...
          <div className="space-y-2">
            ...
            />
            <p className="text-[11px] text-muted-foreground">New here? Create an account in 30 seconds.</p>
          </div>

          <Button className="w-full" onClick={handleEmailLogin} disabled={submitting}>
